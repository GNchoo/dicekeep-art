(function () {
  ...
  let hooks = null, cache = null, raw = null, generation = 0, acting = false, productPrices = new Map();
  ...
  function normalize(data, linked, price) {
    ...
    return { account: { linked, label: linked ? '로그인 계정' : '게스트' }, serverNow: data.serverNow,
      attendance: { claimedToday: !r.attendance.canClaim, nextDay: r.attendance.canClaim ? r.attendance.cycleDay : (r.attendance.total - 1) % 7 + 1,
        claimedCount: r.attendance.total, rewards: r.attendance.cycle, xp: L.ATTENDANCE_XP },
  ...
  async function list() {
    ...
    if (version === generation) { raw = data; cache = view; account = who; }
    return view;
  }
  ...
  async function buyPass() {
    ...
    try {
      ...
      await C.buy(product.sku); return await list();
    } finally { acting = false; }
  }
