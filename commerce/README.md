...
- `POST /runs/resume {ticket}`: 자기 계정의 아직 활성인 티켓만 확인합니다. 시작 시각·덱·레벨을 새로 만들지 않고 원래 snapshot을 반환합니다. 정산되었거나 다른 런으로 대체된 티켓은 거절합니다. 기기의 전투 저장을 서버로 업로드하거나 지급 근거로 신뢰하는 API가 아닙니다.
