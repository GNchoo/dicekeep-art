...
- `buildArenaLayout()` — 무한 투기장 전용: 입구 길 + 보드 밀착 둥근 사각형 트랙(모서리 호 10° 샘플) × `ARENA_LAPS`(2) 바퀴 + 반 바퀴 + 출구 길을 이어 붙인 경로 하나 + 3×5 보드 석단 15개 + `roads`(그리기용) + `board`/`track`(`mid` 포함) 사각형 + `noGoal`/`laps`. (`buildLayout` 은 `map.airPts` 가 있으면 하늘길로 우선 쓰는 분기를 갖고 있지만 현재 아레나는 주지 않는다.)
