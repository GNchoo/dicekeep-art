...
| 사거리 | 인피니티 보드 보정 `INFINITY.rangeBonus = 24` (`towerRange`) → 15칸 전부 트랙에 닿는다 |
...
**보물상자 갓챠 + 다면체 주사위 + 성(★) 타워** — 인피니티의 운 요소. `content.js` `INFINITY.chest`, `game.js` `buyChest`/`rollBagDie`/`finishSlot`, 훅 `DKchest()`·`DKbag()`·`DKrollBag(kind)`, `DIE.forceFinal` 로 결과 강제.
