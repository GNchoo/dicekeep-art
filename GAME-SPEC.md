...
- `buildArenaLayout()` — 무한 투기장 전용: 보드 밀착 둥근 사각형 트랙 폴리라인 하나(모서리 호 10° 샘플) + 3×5 보드 석단 15개 + `board`/`track` 사각형(바닥·연석·화로 연출용). (`buildLayout` 은 `map.airPts` 가 있으면 하늘길로 우선 쓰는 분기를 갖고 있지만 현재 아레나는 주지 않는다.)
