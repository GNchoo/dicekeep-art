...
| 맵 | `cInf` 무한 투기장 — **랜덤다이스식 보드**: 가운데 3×5 석단 보드(15개, 가로 88·세로 88 간격, 중심 512,300)와 둘레를 도는 둥근 사각형 트랙(좌 222·우 802·상 132·하 468, 모서리 60). 왼쪽 변 가운데가 열려 아래 포탈(222,360)에서 출발한 적이 시계 반대 방향으로 한 바퀴 돌아 위 크리스탈(222,240)에 닿는다. 배경은 바닥 그림(`casual/maps/map-inf-arena.jpg`, 없으면 코드 바닥 + 돌 단 보드)만 쓰고 트랙·석단·포탈·크리스탈은 코드가 그린다 (`buildArenaLayout`, `buildRoadLayer`, `drawArenaFloor`) |
| 레인 | 흙길 = 트랙 한 바퀴(≈1,600px) + 지름길 흙길2 = 오른쪽 위 모서리 포탈 → 위 변 → 크리스탈(≈700px) + 하늘길 = 포탈 → 보드 중심 → 크리스탈 V자(≈590px, `map.airPts`) + 땅굴(지름길 옆, ≈750px). 석단 15 (보드, 코드 받침) |
...
- `buildArenaLayout()` — 무한 투기장 전용: 둥근 사각형 트랙 폴리라인(모서리 호 10° 샘플) + 지름길 + V자 하늘길(`airPts`, `buildLayout` 이 있으면 우선 사용) + 3×5 보드 석단 15개 + `board` 사각형(바닥 연출용).
