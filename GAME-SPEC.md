...
| 맵 | `cInf` 무한 투기장 — **랜덤다이스식 보드**: 가운데 3×5 석단 보드(15개, 가로 88·세로 88 간격, 중심 512,300)와 둘레를 도는 둥근 사각형 트랙 **하나**(좌 222·우 802·상 132·하 468, 모서리 60). 왼쪽 변 가운데가 열려 아래 포탈(222,360)에서 출발한 적이 종류와 상관없이 시계 반대 방향으로 한 바퀴 돌아 위 크리스탈(222,240)에 닿는다. 배경은 바닥 그림(`casual/maps/map-inf-arena.jpg`, 없으면 코드 바닥 + 돌 단 보드)만 쓰고 트랙·석단·포탈·크리스탈은 코드가 그린다 (`buildArenaLayout`, `buildRoadLayer`, `drawArenaFloor`) |
| 레인 | **트랙 하나**(≈1,600px). `INFINITY.tier.lanes = ['ground']` 이라 공중·땅굴 적도 `laneFor()` 폴백으로 같은 트랙을 돈다 (랜덤다이스와 같음). 공중 적은 떠서 그려지고 대공 불가 타워(2·6눈)가 못 때리는 규칙은 그대로. 석단 15 (보드, 코드 받침) |
...
- `buildArenaLayout()` — 무한 투기장 전용: 둥근 사각형 트랙 폴리라인 하나(모서리 호 10° 샘플) + 3×5 보드 석단 15개 + `board` 사각형(바닥 연출용). (`buildLayout` 은 `map.airPts` 가 있으면 하늘길로 우선 쓰는 분기를 갖고 있지만 현재 아레나는 주지 않는다.)
