...
window.DKCONTENT = (function () {
  ...
  // o 기본 석단  + 추가 석단 후보(티어별로 2·4·6·8 개 열림)  ~ 물(석단·소품 금지)  T 큰 소품 고정
  // 규칙: 흙길은 4방향으로 이어진 한 줄(옆 줄과 붙지 않게 한 칸 띄움), E 위 칸은 비워 둔다(성 그림 자리), 두 번째 길은 흙길 한 칸에만 닿는다.
  ...
  function buildGridLayout(rows, mirror) {
    ...
    const spots = [], extras = [], water = [], props = [];
    for (let r = 0; r < GH; r++) for (let c = 0; c < GW; c++) {
      const ch = grid[r][c];
      if (ch === 'o') spots.push(cellCenter(r, c));
      else if (ch === '+') extras.push(cellCenter(r, c));
      else if (ch === '~') water.push([r, c]);
      else if (ch === 'T') props.push([r, c]);
    }
    // 추가 석단은 "이미 고른 것들에서 가장 먼 후보" 순으로 정렬 → 앞에서 n 개만 열어도 고르게 퍼진다
    const ordered = [];
    const pool = extras.slice();
    while (pool.length) {
      let best = 0, bestD = -1;
      for (let i = 0; i < pool.length; i++) {
        const ref = ordered.length ? ordered : spots;
        let d = Infinity;
        for (const s of ref) d = Math.min(d, Math.hypot(pool[i][0] - s[0], pool[i][1] - s[1]));
        if (!ref.length) d = pool[i][0] + pool[i][1];
    ...
    return {
      ...
      spots, spots2: ordered, water, props,
