(function () {
  ...
  const state = { api: null, view: null, tab: 'attendance', loading: false, busy: false, error: '', notice: '', generation: 0, context: 0 };
  ...
  function create() {
    ...
    const header = el('header', 'rw-header'), titles = el('div');
    titles.append(el('p', 'rw-eyebrow', 'DICEKEEP'), el('h2', '', '성장의 기록')); titles.lastChild.id = 'rewards-title';
  ...
  function renderAttendance(panel, view) {
    ...
    panel.append(heading('놓친 날이 있어도 괜찮아요', '한국 시간 자정에 새 출석이 열립니다. 접속하지 않은 날에는 출석 순서가 초기화되지 않아요.'));
    ...
    (attendance.rewards || daily.map(([gold, shards]) => ({ gold, shards }))).forEach((reward, index) => {
      ...
      const card = el('article', 'rw-day' + (current ? ' rw-day-current' : '') + (received ? ' rw-day-done' : ''));
      card.dataset.day = day; card.append(el('h4', '', day + '회차'), el('span', 'rw-day-mark', received ? '✓' : day === 7 ? '✦' : '◇'), rewards(reward), el('p', 'rw-day-state', received ? '수령 완료' : current ? '이번 출석' : '다음 출석'));
      grid.append(card);
    }); panel.append(grid);
  ...
  function renderMail(panel, view) {
    panel.append(heading('성채에 도착한 선물', '보상을 받으면 계정에 바로 반영됩니다. 만료일이 있는 우편은 기간 안에 수령해 주세요.'));
    if (view.canAdmin) { const admin = button('운영 우편 관리', () => window.DKMAILADMIN?.open()); admin.id = 'rewards-mail-admin'; panel.append(admin); }
    if (!view.account?.linked) { const empty = el('div', 'rw-empty'); empty.append(el('span', 'rw-empty-mark', '✉'), el('h4', '', '운영 우편은 로그인 후 확인해요'), el('p', 'rw-detail', '게스트도 출석과 무료 성장 패스를 이용할 수 있습니다. 운영자가 보내는 우편은 로그인한 계정으로 도착합니다.')); panel.append(empty); return; }
    const items = view.mail || [];
    if (!items.length) { const empty = el('div', 'rw-empty'); empty.append(el('span', 'rw-empty-mark', '✉'), el('h4', '', '새 우편이 없습니다'), el('p', 'rw-detail', '성채에 선물이 도착하면 이곳에서 확인할 수 있어요.')); panel.append(empty); return; }
    const list = el('div', 'rw-mail-list');
    items.forEach(mail => {
      const expired = typeof mail.expired === 'boolean' ? mail.expired : (!!mail.expiresAt && +new Date(mail.expiresAt) <= Date.now()), card = el('article', 'rw-mail'); card.dataset.mailId = mail.id;
      card.append(el('h4', '', mail.title || '성채 우편'), el('p', 'rw-mail-body', mail.body || ''), rewards(mail.reward || mail));
      card.append(el('p', 'rw-mail-dates', '발행 ' + date(mail.createdAt) + ' · 만료 ' + date(mail.expiresAt) + ' (한국 시간)'));
      ...
      card.append(receive); list.append(card);
    }); panel.append(list);
  }
  function renderPass(panel, view) {
    ...
    panel.append(heading('기한 없이 쌓이는 성장', '총 20단계 · 단계마다 100 XP. 출석과 플레이로 경험치를 모으고, 달성한 보상을 직접 받아요.'));
    panel.append(el('p', 'rw-detail', '출석 20 XP · 투기장 완료 웨이브마다 2 XP · 대전·협동 유효 참여 1분당 15 XP. 전투 XP는 종료 정산 때 반영하며 캠페인은 제외됩니다.'));
    ...
    const bar = el('progress'); bar.max = maxXp; bar.value = Math.min(xp, maxXp); bar.setAttribute('aria-label', '성장 패스 경험치'); progress.append(caption, bar); panel.append(progress);
    const offer = el('div', 'rw-pass-offer'); offer.append(el('strong', '', pass.premiumOwned ? '프리미엄 패스 보유 중' : '프리미엄 · 선택 구매'), el('p', 'rw-detail', '기한 없음 · 한 번 구매 · 자동 갱신 없음. 구매 전에 달성한 단계도 소급 수령합니다. 다이스 해금은 무료 플레이로 가능합니다.'));
    ...
    tiers.forEach(tier => {
      const row = el('article', 'rw-pass-tier'); row.dataset.tier = tier.tier;
      ...
      ['free', 'premium'].forEach(track => {
        ...
        const cell = el('div', 'rw-pass-reward' + (track === 'premium' ? ' rw-premium' : ''));
        ...
        cell.append(rewards(reward), claim); row.append(cell);
      }); list.append(row);
    }); panel.append(list);
  }
