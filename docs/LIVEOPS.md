...
보물 주머니·7회차 상자·밀랍 편지·성장 기록부·성장 결정 아이콘을 `ui/rewards/`에서 불러온다. 출석 회차·골드·조각 수량과 수령 상태는 그림과 별도로 표시한다. 무료·프리미엄 패스는 동일 단계의 두 열로 비교하며 수령 완료·미달성·구매 필요·수령 가능 상태를 구분한다. 자세한 XP와 구매 안내는 펼침 영역으로 제공하며 화면 갱신에도 펼친 상태를 유지한다. 이미지·레이아웃 변경은 지급 수치나 구매 권한을 변경하지 않는다. [제작 도구·정확한 프롬프트](REWARD-ART.md).
