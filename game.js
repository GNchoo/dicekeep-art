...
function buildWave(w) {
  ...
  for (let i = 0; i < n; i++) {
    let pool = ['slime', 'shroom', 'pig', 'chicken', 'goblin', 'sheep', 'penguin', 'squirrel', 'frog', 'duck', 'koala', 'hedgehog', 'goat', 'otter', 'tanuki', 'wolf', 'mouse', 'alpaca', 'seahorse', 'beaver', 'kiwi', 'snake', 'capybara', 'axolotl', 'meerkat', 'quokka'];
    if (w >= 4) pool = pool.concat(['cactus', 'fox', 'raccoon', 'turtle', 'panda', 'catsamurai', 'boar', 'chameleon', 'porcupine', 'rhino', 'hippo', 'lemur', 'sloth']);
    if (unlockAir) pool = pool.concat(['bee', 'balloon', 'bat', 'owl', 'parrot', 'dragonfly', 'cloudsheep', 'humming', 'ladybug', 'fairy', 'crane', 'pigeon', 'moth', 'firefly', 'pelican', 'butterfly', 'jellyfish', 'toucan', 'flamingo', 'eagle', 'dragoncub', 'squid']);
    if (unlockBurrow) pool = pool.concat(['mole', 'worm', 'arma', 'beetle', 'crab', 'prairie', 'rabbit', 'badger', 'chipmunk', 'wormknight', 'vole', 'pangolin', 'ant', 'gecko', 'wombat', 'ferret', 'centipede', 'shrew']);
