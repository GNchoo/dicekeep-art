...
function buildWave(w) {
  ...
  for (let i = 0; i < n; i++) {
    let pool = ['slime', 'shroom', 'pig', 'chicken', 'goblin', 'sheep', 'penguin', 'squirrel', 'frog', 'duck', 'koala', 'hedgehog'];
    if (w >= 4) pool = pool.concat(['cactus', 'fox', 'raccoon', 'turtle', 'panda']);
    if (unlockAir) pool = pool.concat(['bee', 'balloon', 'bat', 'owl', 'parrot', 'dragonfly', 'cloudsheep', 'humming', 'ladybug']);
    if (unlockBurrow) pool = pool.concat(['mole', 'worm', 'arma', 'beetle', 'crab', 'prairie', 'rabbit', 'badger', 'chipmunk']);
