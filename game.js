...
function buildWave(w) {
  ...
  for (let i = 0; i < n; i++) {
    let pool = ['slime', 'shroom', 'pig', 'chicken', 'goblin', 'sheep', 'penguin', 'squirrel', 'frog', 'duck', 'koala', 'hedgehog', 'goat', 'otter', 'tanuki'];
    if (w >= 4) pool = pool.concat(['cactus', 'fox', 'raccoon', 'turtle', 'panda', 'catsamurai']);
    if (unlockAir) pool = pool.concat(['bee', 'balloon', 'bat', 'owl', 'parrot', 'dragonfly', 'cloudsheep', 'humming', 'ladybug', 'fairy', 'crane']);
    if (unlockBurrow) pool = pool.concat(['mole', 'worm', 'arma', 'beetle', 'crab', 'prairie', 'rabbit', 'badger', 'chipmunk', 'wormknight', 'vole']);
