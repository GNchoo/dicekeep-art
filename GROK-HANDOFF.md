...
**현재 상태의 단일 소스: [GAME-SPEC.md](GAME-SPEC.md)**  
파일 목록: [ASSET-MANIFEST.md](ASSET-MANIFEST.md)

이 문서는 초기 P0~P2 프롬프트와 연결 방법을 보관한다.
사람(제작자)에게 묻기 전에 `GAME-SPEC.md` → `game.js` → `content.js` 를 먼저 읽어라.
대화는 **한국어**, 이미지 프롬프트는 **영어**.
...
이미 동작하는 Canvas TD + 캐주얼 아트팩 + 로비/상점 + 5 난이도 티어.

- 눈 1~6 = 타워. 6이 최강. 같은 눈 합체 Lv3.
- 드래그 = 필드 3D 물리 굴림 / 버튼·R = ?박스 안 빠른 굴림. **합치지 말 것.**
- 인게임 타워는 `casual/towers/t1-a.png` ~ `t6-a.png` (2D 아이소 건물). `tN-b`~`e`는 상점 스킨 (재생성 예정).
- 공격 시트 `tN-attack-2x2.png`. VFX는 `vfx/`.
- 맵별 PATH/SPOTS: **50맵 전부 작성 완료** (`MAP_LAYOUTS`). 하드 티어용 흙길2/추가석단은 `MAP_LAYOUTS_HARD` (하드 배경 생성 후 작성).
- 티어 1~5: 흙길 → +하늘길 → +땅굴 → +흙길2 → 전부. 하늘길·땅굴·추가 석단은 코드 생성 (`content.js` `buildLayout`).
- 카탈로그: 맵 50, 적 500, 보스 100, 걷기 시트 12종 (13~24·보스 1~10 은 코드 선연결, 파일 대기).

다음: **`ART-PROMPTS.md`** 의 하드 배경 20 / 걷기 시트 22 / 스킨 24 생성. 자세한 목록은 GAME-SPEC §8.

아래 §1~§7 은 초기 브리프 원문이다. 현재 상태와 다른 부분(타워 합성 스프라이트, 맵 1장 등)은 GAME-SPEC 이 우선한다.
