...
이미 동작하는 Canvas TD + 캐주얼 아트팩.

- 눈 1~6 = 타워. 6이 최강. 같은 눈 합체 Lv3.
- 드래그 = 필드 3D 물리 굴림 / 버튼·R = ?박스 안 빠른 굴림. **합치지 말 것.**
- 인게임 타워는 `casual/towers/t1-a.png` ~ `t6-a.png` (2D 아이소 건물). `tN-b`~`e`는 보존만.
- 공격 시트 `tN-attack-2x2.png`. VFX는 `vfx/`.
- 맵별 PATH/SPOTS: 기본 S커브, 맵10 대나무·맵13 태엽은 오버라이드.
- 카탈로그: 맵 50, 적 500, 보스 100, 걷기 시트 12종.

다음: 나머지 맵 경로 맞춤, 적 걷기 시트 추가. 자세한 목록은 GAME-SPEC §8.
