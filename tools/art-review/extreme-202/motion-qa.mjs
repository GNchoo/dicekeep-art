// Snapshot of shared motion checks tools/preview-directional-motion.mjs SHA256 8d27b7a9b7b07743cb1072ae67107c0a859c297faeb70aba1a9b7547aab55d36; identical checks for all111 entries, only8 representative moving GIF encodes.
...
const gifIds = new Set(['w102','w123','w130','w133','w138','b171','w183','b201']);
...
for (const [id, entry] of Object.entries(manifest.entries)) {
  ...
  if (!gifIds.has(id)) {
    report.entries.push({assetId:id,locomotion:entry.locomotion,phaseDriver:distanceDriven?'distance':'time',reviewTravelCanonical,gif:null,gifScope:'Per-direction real frame GIFs remain in builder output. This entry receives all pose/contact checks but no duplicate large moving GIF.',motionFrames:count*cycles,checks});
    console.log('checked',id,checks.length+' traces (no duplicate moving GIF)');continue;
  }
  const outputFrames = [];
  for (let i = 0; i < count * cycles; i++) {
