// Build new extreme bodies with existing approved articulated limbs into fixed-cell assets.
// node tools/build-extreme-art.mjs --config=... [--out=gen/directional-art] [--only=w001,b020-2]
...
const out = path.resolve(repo, opt('out', 'gen/extreme-202'));
...
const report = { version: 1, config: path.relative(repo, path.resolve(repo, configFile)).replaceAll('\\', '/'), configSha256: sha256(fs.readFileSync(path.resolve(repo, configFile))), canonicalCell,
  scope: 'Geometry/image integrity checks; human anatomy/art approval remains separate. Runtime encodes smaller lossless WebP only after visible RGBA equality.', entries: [], files: [], compression: [], errors: [] };
const resumed = new Map();
if (args.includes('--resume')) {
  const prior = JSON.parse(fs.readFileSync(path.join(out,'directional-qa.json'))), inputBytes = fs.readFileSync(path.join(out,'build-input.json'));
  if (sha256(inputBytes) !== prior.configSha256 || prior.canonicalCell !== canonicalCell) throw new Error('resume config provenance mismatch');
  const old = expandConfig(JSON.parse(inputBytes));
  for (const entry of entries) {
    const original = old.entries.find(e=>e.assetId===entry.assetId), result = prior.entries.find(e=>e.assetId===entry.assetId);
    if (!original || !result || JSON.stringify(original)!==JSON.stringify(entry)) continue;
    const prefix=`casual/${entry.role==='normal'?'enemies':'bosses'}/extreme/${entry.assetId}-`;
    const files=prior.files.filter(f=>['side','front','back'].some(v=>f.file.startsWith(prefix+v+'-walk-')||f.file===prefix+v+'.webp'||f.file===prefix+v+'.png'||f.file===`review/${entry.assetId}-${v}.png`||f.file===`review/${entry.assetId}-${v}.gif`));
    if (files.length!==12 || files.some(f=>sha256(fs.readFileSync(path.join(out,f.file)))!==f.sha256)) throw new Error('resume image provenance mismatch: '+entry.assetId);
    resumed.set(entry.assetId,{result,files,compression:prior.compression.filter(c=>c.label.startsWith(entry.assetId+' '))});
  }
}
fs.mkdirSync(out,{recursive:true});fs.copyFileSync(path.resolve(repo,configFile),path.join(out,'build-input.json'));
...
const encodeRuntime = async (png, label) => {
  ...
  report.compression.push({ label, pngBytes: png.length, encodedBytes: useWebP ? webp.length : png.length, pngSha256: sha256(png), encoding: useWebP ? 'lossless-webp' : 'png', visibleRGBADifferences: 0, hiddenRGBDifferences });
  return { bytes: useWebP ? webp : png, extension: useWebP ? '.webp' : '.png' };
};
try {
  const page = await browser.newPage(); await page.goto('about:blank');
  for (const { entry, views, identities } of prepared) {
    const runtime = { assetId: entry.assetId, wave: entry.wave, role: entry.role, name: entry.name, legacyAssetId: entry.legacyAssetId, baseRigId: entry.baseRigId, ready: false, locomotion: entry.locomotion,
    ...
    const cached=resumed.get(entry.assetId);
    if(cached){
      for(const[name,rig]of Object.entries(views)){
        const prefix=`casual/${entry.role==='normal'?'enemies':'bosses'}/extreme/${entry.assetId}-${name}`;
        const still=cached.files.find(f=>f.file===prefix+'.webp'||f.file===prefix+'.png').file;
        const sheet=cached.files.find(f=>f.file.startsWith(prefix+'-walk-')).file;
    ...
    for (const [name, rig] of Object.entries(views)) {
      ...
      const prefix = `casual/${entry.role === 'normal' ? 'enemies' : 'bosses'}/extreme/${entry.assetId}-${name}`;
      let sheetFile = prefix + `-walk-${cols}x${rows}`, stillFile = prefix;
      ...
      const encodedSheet = await encodeRuntime(sheet, entry.assetId + ' ' + name + ' sheet'), encodedStill = await encodeRuntime(still, entry.assetId + ' ' + name + ' neutral');
      sheetFile += encodedSheet.extension; stillFile += encodedStill.extension;
      await recordFile(sheetFile, encodedSheet.bytes); await recordFile(stillFile, encodedStill.bytes);
...
fs.writeFileSync(path.join(out, 'directional-art.json'), JSON.stringify(output, null, 2) + '\n');
fs.writeFileSync(path.join(out, 'directional-art.js'), 'window.INF_EXTREME_ART = ' + JSON.stringify(output, null, 2) + ';\n');
