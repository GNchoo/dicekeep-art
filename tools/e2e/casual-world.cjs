...
(async()=>{const browser=await launchBrowser();try{
 ...
 for(const viewport of [{width:1100,height:800},{width:390,height:844}]){
  ...
  const art=await page.evaluate(()=>{
   ...
   return {sizes,cleanSizes,loaded,starTowers,paleAlpha:pixel[3]};
  });
 ...
 fs.writeFileSync(`${out}/report.json`,JSON.stringify(reports,null,2));console.log('PASS casual world: promoted art and 14 unique 7–20★ tower sprites, authored alpha, both tower variants, six regions on phone and desktop');
