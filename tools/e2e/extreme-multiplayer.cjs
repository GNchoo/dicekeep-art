...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(await a.evaluate(()=>{let result;__mpRecovery.withView(()=>{const t=DK.towers.find(t=>t.face===6);result={localResearch:DK.inf.growthSnapshot.awakenings[6],awake:__mpRecovery.towerAwakened(t),aura:__mpRecovery.drawAwakeningAura(t),labels:__mpRecovery.badgeLabels(t)};});return result;}),{localResearch:true,awake:false,aura:false,labels:['★6']},'real spectator draw preserves numeric labels and never infers a remote awakening aura from the local account');
    report.checks.push('spectator preserves numeric labels without an awakening aura for an unresearched remote seven-pip income tower despite local awakening research');
