// Isolated local game: physical attack data is unchanged; visible attacks share the muzzle.
...
(async()=>{const browser=await launchBrowser();try{
 ...
 const rows=await page.evaluate(()=>{
  ...
  const p=epos(e),rows=[];
  for(const face of [1,2])for(const serial of [0,1]){
   ...
   rows.push({face,serial,origin,expected,beam:DK.beams[0]?.pts[0],style:DK.beams[0]?.style,
    muzzle:DK.fxs.find(f=>f.kind==='muzzleFlash'),projectile:DK.projs[0]?{x:DK.projs[0].x,y:DK.projs[0].y,offset:DK.projs[0].launchOffset}:null,
    physical:{x:t.x,y:t.y-64},roofFlash:DK.fxs.some(f=>f.kind==='laserMuzzle')});
  }
  return rows;
 });
 for(const r of rows){assert.deepEqual(r.origin,r.expected);if(r.face===1){assert.deepEqual(r.beam,r.origin);assert.equal(r.style,'rubyShot');assert.equal(r.roofFlash,false);}else{assert.equal(r.muzzle.x,r.origin.x);assert.equal(r.muzzle.y,r.origin.y);assert.equal(r.projectile.x,r.physical.x);assert.equal(r.projectile.y,r.physical.y);assert.equal(r.projectile.x+r.projectile.offset[0],r.origin.x);assert.equal(r.projectile.y+r.projectile.offset[1],r.origin.y);}}
 ...
 console.log('PASS lens origin, ruby shot, alternating cannon muzzle and unchanged projectile physics');
