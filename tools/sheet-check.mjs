...
console.log(`자세 IoU [${mat}] 이웃 최소 ${pose.adjacentMin.toFixed(2)}${pose.flags.length ? '  ← ' + pose.flags.map((f) => f === 'static' ? '거의 안 움직임 (재생성)' : '1≈3·2≈4 (정상 주기도 이렇게 나옴 — 눈으로 확인)').join(', ') : '  · 보행 주기로 보임'}`);
...
process.exit(pose.flags.includes('static') ? 1 : 0);
