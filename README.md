...
- 50 스테이지를 모두 깨면 **인피니티 모드**(무한 투기장): 랜덤다이스식 보드(석단 15) + 둘레 트랙 하나(모든 적이 같은 길), **6눈·석단 전부 해금된 풀파워**로 끝없이 강해지는 웨이브를 얼마나 오래 버티는지 겨룬다. SP로 눈별 강화, 골드로 **보물상자**를 사면 다면체 주사위(d1~d20)가 나오고 굴린 숫자만큼의 성(★) 타워(7~20성 히든)가 선다. 최고 웨이브 기록·젬 보상
