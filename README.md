...
- **인피니티 · 함께**(멀티): 로비에서 이름을 넣고 **방 만들기** → 6자리 코드를 친구에게 → **코드로 참가** → 2명 이상 모이면 방장이 시작. 2~4명이 **각자 보드에서 같은 웨이브**를 동시에 맞는다(도전 규칙). 첫 웨이브 전 20초 준비. **방은 아무도 기다리지 않고**(보스 웨이브만 전원 처치까지 최대 5분 20초) 자리를 비우면 돌아왔을 때 밀린 웨이브를 이어서 맞는다. 먼저 죽으면 관전(기록·젬은 그 즉시 저장), 101웨이브 완주 = 클리어, 판이 끝나면 순위표. 가로는 오른쪽에, 세로는 아래에 상대 보드 요약 카드가 뜬다. `Enter` 로 채팅. 빠른 매칭·재대전·초대 링크는 다음 단계
