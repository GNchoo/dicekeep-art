...
현재 투기장은 **순수운빨(101·성장 미적용)**, **덱빌드(101·편성 5종·Lv20)**, **극한(무한·Lv200)**으로 나뉜다. 모든 투기장 모드에서 무료 성장 조각을 얻는다. [규칙·보상·성능 측정](docs/MODES-AND-PERFORMANCE.md)에 상세 조건과 검증 방법이 있다. [개발 진행](docs/DEVELOPMENT-STATUS.md)과 [외형·소액 성장 상품](docs/BUSINESS-MODEL.md)에 최신 변경과 출시 조건을 기록한다.

웹/Android 결제 연동은 기본 비활성화 상태다. [계정·결제 서버](commerce/README.md)와 [Android 구매 연동](docs/ANDROID-PAYMENTS.md)의 설정 및 테스트를 마쳐야 실판매가 가능하다. 키와 계정이 준비되지 않은 상태를 결제 출시 완료로 보지 않는다.
...
7. `https://dicekeep-net.<계정>.workers.dev/health` 의 protocol이 4인지 확인한다. 클라이언트와 서버 버전을 함께 반영해야 한다. 게임은 자기 주소에서 멀티 서버 주소를 만든다. 커스텀 도메인은 `net/wrangler.jsonc`의 `vars.ALLOWED_ORIGINS`에 넣는다.
...
- **인피니티 모드**(무한 투기장)는 스테이지 진행과 무관하게 **처음부터 바로** 할 수 있다. 고정 보드(석단 15) + 둘레 트랙 하나(모든 적이 같은 길), **6눈·석단 전부 해금된 풀파워**. 아레나 캔버스는 **화면 비율로 만들어져 레터박스 없이 화면을 다 쓴다** — 세로면 보드가 3열×5행으로 서고 HUD 두 줄이 아래에, 가로면 5열×3행에 HUD 한 줄이 아레나 아래쪽에 반투명으로 겹친다. 런 도중 돌리거나 리사이즈해도 상태가 유지된다. 골드로 **뽑기**(160G)를 하면 먼저 등급이 정해진다. 4면체 이상은 직접 던지고, 멈췄을 때 금색으로 표시되는 면의 숫자대로 성(★) 타워(7~20성 히든 포함)를 얻는다. 골드로 눈(1~6)을 **파워업**한다. 세 가지 모드 — **순수운빨**(기존 101웨이브), **덱빌드**(5종 편성·101웨이브), **극한**(성장·무한 웨이브). 첫 런에는 조작을 단계별로 안내한다
...
준비(Windows): Node LTS + Android Studio(SDK 36 + JDK 21). 그 다음:

```bash
npm i                                  # Capacitor · sharp · 글꼴
npm run fonts                          # fonts/ 갱신 (Google Fonts 대신 로컬 글꼴, 웹도 같이 씀)
npm run build:www -- --optimize        # www/ 생성 (최종 용량은 빌드 manifest 확인)
...
순수운빨은 무과금·과금의 영향이 전혀 없어야 하고, 난이도는 기준 리비전과 같아야 한다(의도한 변경 제외). 두 성질을 각각 실제 게임으로 확인한다.
...
- **현재 방향 아트·보스**: 위 명령은 초기 단방향 시트 도구다. 현재 승인된 일반 몬스터와 보스·부관은 `directional-art.js`와 `extreme-art.js`의 측면·정면·후면 시트를 사용하며, 반대 측면은 반사한다. 지상·뱀형은 8프레임, 비행·부유형은 4프레임이다. 공통 `referenceHeight`·pivot·scale을 사용해 방향 전환 때 임의로 크기를 재정규화하지 않는다. 보행 시트를 기다리거나 로드에 실패하면 같은 캐릭터의 정지컷·내장 fallback을 사용한다. 논리 전투 크기와 표시 높이는 별개이며 보스의 기본 표시 크기와 부관 배율은 기존 규칙을 유지한다. 제작·검증 계약은 [DIRECTIONAL-RIG.md](tools/DIRECTIONAL-RIG.md)를 따른다.
...
독점 저작물입니다 — 코드·게임 데이터·아트 재사용 허가를 부여하지 않습니다. 자세한 내용과 아트 생성 고지는 [LICENSE](LICENSE).
저장소가 공개라는 사실은 라이선스 부여가 아닙니다.
