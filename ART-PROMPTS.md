...
> **현재 상태 (2026-09-02)**: 아래 세 섹션 모두 **코드로 합성한 임시본이 이미 들어가 있다** (`scratchpad/gen-hard.js`, `gen-walk.js`, `gen-skins.js`).
> - 하드 배경 20장: 기존 배경 위에 A* 로 찾은 두 번째 흙길을 그려 넣은 것. `MAP_LAYOUTS_HARD` 에 `src`/`path2`/`spots2` 까지 연결돼 티어 4·5 에서 실제로 두 갈래 흙길이 동작한다.
> - 걷기 시트 34장: 대기컷 1장을 기울기·바운스로 4프레임화한 것.
> - 스킨 24장: `tN-a` 를 테마별로 재채색한 것.
>
> Grok 으로 생성한 진짜 아트가 나오면 **같은 파일명으로 덮어쓰기**만 하면 된다. 하드 배경만은 길 모양이 달라지므로 덮어쓴 뒤 에디터로 `path`/`path2`/`spots`/`spots2` 를 다시 찍어야 한다.
...
## §1. 하드 배경 20장 (스테이지 31~50, 티어 4·5)

티어 4·5는 **두 번째 흙길(흙길2)** 을 쓴다. 지금은 코드가 그린 임시 흙길(`-hard.jpg`)이 들어가 있다. 아래 배경을 생성해 덮어쓰면 그림이 어울리는 두 갈래 길이 된다.
