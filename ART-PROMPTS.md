...
| 섹션 | 수량 | 저장 위치 | 코드 연결 |
|---|---|---|---|
| §0 테마 타일셋 | 90 | `casual/tiles/<theme>/*.png` | **현행** — 파일만 넣으면 됨 (`GROK-BRIEF.md`) |
...
## §0. 테마 타일셋 90장 (현행 방식, 2026-09-02 전면 변경)

**맵 그림에 좌표를 맞추던 방식은 폐기.** 맵은 `content.js` 의 16×9 ASCII 템플릿으로 코드가 설계하고(`buildGridLayout`), 테마별 타일 조각을 `game.js` `buildTileLayer` 가 씌운다. 조각이 없으면 그 자리만 코드로 그리므로 부분 납품도 된다.
...
| `road-straight/corner/t/cross.png` | 1024² | 64×64, 90° 회전 | 길 폭 56%, 가장자리에서 딱 잘림, 바깥은 #C8C8C8 단색, 평평한 조명 |
| `water.png` | 1024² | 64×64 | 가장자리까지 물, 반복해도 이어지게 |
...
품질 점검 포인트: (1) 직선·코너 타일을 이어 붙였을 때 길 폭이 같은가 (2) 회색이 깨끗이 지워졌는가 (풀·그림자를 회색 위에 그리면 잔상이 남는다) (3) 6테마의 선 굵기·채도가 같은가.
