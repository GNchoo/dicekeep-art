(function (root, factory) {
  ...
  function snapshot(profile, mode) {
    ...
    const levels = {};
    for (let face = 1; face <= 20; face++) levels[face] = profile.levels[face];
    return Object.freeze({ mode, growth: growthMode(mode), levelCap: levelCap(mode), deck: Object.freeze(profile.deck.slice()), levels: Object.freeze(levels) });
  }
  ...
  return Object.freeze({ VERSION, MAX_SHARDS, MAX_LEVEL, MAX_COUNTER, DECK_SIZE, MODES, MILESTONES, GEM_MILESTONES,
    defaultProfile, sanitize, normalizeRecord, cardUnlockCost, upgradeCost, unlock, upgrade, setDeck, snapshot, damageMultiplier, draw, settle });
});
