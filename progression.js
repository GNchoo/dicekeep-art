(function (root, factory) {
  ...
  const BATTLE_REWARD_VERSION = 1, BATTLE_MAX_SECONDS = 6000;
  const BATTLE_REWARDS = Object.freeze({ goldPerMinute: 300, shardsPerMinute: 8, minVictorySeconds: 60,
    win: Object.freeze({ gold: 100, shards: 5 }), firstWin: Object.freeze({ gold: 300, shards: 20 }),
    activity: Object.freeze([[10,300,20],[30,400,30],[60,500,40],[120,600,40],[240,800,50],[480,1000,60]]
      .map(([minutes,gold,shards]) => Object.freeze({ minutes, seconds: minutes * 60, gold, shards }))) });
  ...
  const emptyBattleRewards = () => ({ version: BATTLE_REWARD_VERSION, activeSeconds: 0, activityClaimed: [], firstWins: { duel: false, coop: false } });
  function battleRewardsValid(value) {
    return isObject(value) && value.version === BATTLE_REWARD_VERSION && integer(value.activeSeconds, 0, MAX_COUNTER)
      && listValid(value.activityClaimed, BATTLE_REWARDS.activity.map(m => m.minutes))
      && value.activityClaimed.every(minutes => value.activeSeconds >= minutes * 60)
      && isObject(value.firstWins) && ['duel', 'coop'].every(mode => typeof value.firstWins[mode] === 'boolean');
  }
  function sanitizeBattleRewards(raw) {
    const out = emptyBattleRewards(); if (!isObject(raw) || raw.version !== BATTLE_REWARD_VERSION) return out;
    out.activeSeconds = count(raw.activeSeconds);
    out.activityClaimed = BATTLE_REWARDS.activity.filter(m => m.seconds <= out.activeSeconds && Array.isArray(raw.activityClaimed) && raw.activityClaimed.includes(m.minutes)).map(m => m.minutes);
    for (const mode of ['duel', 'coop']) out.firstWins[mode] = raw.firstWins?.[mode] === true;
    return out;
  }
  ...
  function defaultProfile(legacySave) {
    ...
    const profile = { version: VERSION, economyVersion: ECONOMY_VERSION, shards: 0, levels, deck: [1, 2, 3, 4, 5], records, legacy: legacyFrom(legacySave), settled: [], battleRewards: emptyBattleRewards() };
  ...
  function runValid(run, mode) {
    return isObject(run) && idValid(run.id) && MODES.includes(run.mode) && (mode == null || run.mode === mode)
      ...
      && (run.rewardVersion === undefined && run.activeSeconds === undefined || trialMode(run.mode) && run.rewardVersion === BATTLE_REWARD_VERSION
        && integer(run.activeSeconds, 0, Math.min(run.elapsed, BATTLE_MAX_SECONDS))
        && (run.teamKills === undefined || integer(run.teamKills, 0, MAX_COUNTER))
        && (run.matchId === undefined || idValid(run.matchId)) && (run.reason === undefined || idValid(run.reason)));
  }
  ...
  function copyRun(run) {
    return { id: run.id, mode: run.mode, wave: run.wave, kills: run.kills, won: run.won, date: run.date, elapsed: run.elapsed,
      ...(run.rewardVersion === BATTLE_REWARD_VERSION ? { rewardVersion: BATTLE_REWARD_VERSION, activeSeconds: run.activeSeconds,
        ...(run.teamKills !== undefined ? { teamKills: run.teamKills } : {}), ...(run.matchId !== undefined ? { matchId: run.matchId } : {}),
  ...
  function sanitize(raw, legacySave) {
    ...
    out.battleRewards = sanitizeBattleRewards(source.battleRewards);
  ...
  function profileValid(profile) {
    return isObject(profile) && profile.version === VERSION && integer(profile.shards, 0, MAX_SHARDS)
      ...
      && (profile.battleRewards === undefined || battleRewardsValid(profile.battleRewards));
  }
  ...
  function snapshot(profile, mode) {
    ...
    if (mode === 'duel') {
      if (!RULES || !TREE) return null;
      const levels = {}, classes = {}, mastery = {}, talents = {}, awakenings = {};
      for (let face = 1; face <= 20; face++) {
        const owned = profile.collection?.cards?.[face]?.owned || profile.levels[face] > 0;
  ...
  function rewardView(profile) {
    if (!profileValid(profile)) return null;
    const state = profile.battleRewards || emptyBattleRewards();
    const milestones = BATTLE_REWARDS.activity.map(m => ({ ...m, claimed: state.activityClaimed.includes(m.minutes) }));
    const next = milestones.find(m => !m.claimed);
    return { version: BATTLE_REWARD_VERSION, scope: 'battle', activeSeconds: state.activeSeconds, activeMinutes: Math.floor(state.activeSeconds / 60),
      perMinute: { gold: BATTLE_REWARDS.goldPerMinute, shards: BATTLE_REWARDS.shardsPerMinute },
      win: { ...BATTLE_REWARDS.win, minSeconds: BATTLE_REWARDS.minVictorySeconds },
      firstWin: { ...BATTLE_REWARDS.firstWin, minSeconds: BATTLE_REWARDS.minVictorySeconds }, firstWins: { ...state.firstWins }, milestones,
      nextMilestone: next ? { ...next, remainingSeconds: Math.max(0, next.seconds - state.activeSeconds) } : null };
  }
  function battleAward(profile, run) {
    const prior = profile.battleRewards || emptyBattleRewards();
    const total = Math.min(MAX_COUNTER, prior.activeSeconds + run.activeSeconds);
    const first = run.won && run.activeSeconds >= BATTLE_REWARDS.minVictorySeconds && !prior.firstWins[run.mode];
    const won = run.won && run.activeSeconds >= BATTLE_REWARDS.minVictorySeconds;
    const newly = BATTLE_REWARDS.activity.filter(m => total >= m.seconds && !prior.activityClaimed.includes(m.minutes));
    const base = { gold: Math.floor(run.activeSeconds * BATTLE_REWARDS.goldPerMinute / 60), shards: Math.floor(run.activeSeconds * BATTLE_REWARDS.shardsPerMinute / 60) };
    const victory = won ? { ...BATTLE_REWARDS.win } : { gold: 0, shards: 0 };
    const firstWin = first ? { ...BATTLE_REWARDS.firstWin } : { gold: 0, shards: 0 };
    const activity = { milestones: newly.map(m => m.minutes), gold: newly.reduce((n, m) => n + m.gold, 0), shards: newly.reduce((n, m) => n + m.shards, 0) };
    const earned = { gold: base.gold + victory.gold + firstWin.gold + activity.gold, shards: base.shards + victory.shards + firstWin.shards + activity.shards };
    return { state: { version: BATTLE_REWARD_VERSION, activeSeconds: total, activityClaimed: [...prior.activityClaimed, ...activity.milestones],
      firstWins: { ...prior.firstWins, [run.mode]: prior.firstWins[run.mode] || first } },
      detail: { rewardVersion: BATTLE_REWARD_VERSION, activeSeconds: run.activeSeconds, totalActiveSeconds: total, base, victory, firstWin, activity, earned } };
  }
  ...
  function settle(profile, run) {
    ...
    // Versionless trial records remain unrewarded local observations. The new
    // marker and activeSeconds are supplied by the server result bridge; this
    // pure calculator is not a replacement for authenticating that result.
    const trial = trialMode(run.mode);
    const verifiedBattle = trial && run.rewardVersion === BATTLE_REWARD_VERSION;
    if (verifiedBattle && !collectionValid(profile.collection)) return rejected('collection-required');
    const battle = verifiedBattle ? battleAward(profile, run) : null;
    ...
    const earnedShards = battle ? battle.detail.earned.shards : trial ? 0 : Math.floor(Math.min(run.wave, 500) / 5) * 5 + newly.length * 10 + clearBonus;
    ...
    if (battle) profile.battleRewards = battle.state;
    const collectionRewards = { gold: 0, packs: 0 };
    if (battle) {
      collectionRewards.gold = Math.min(battle.detail.earned.gold, MAX_GOLD - profile.collection.gold);
      profile.collection.gold += collectionRewards.gold;
      battle.detail.credited = { gold: collectionRewards.gold, shards };
    } else if (!trial && collectionValid(profile.collection)) {
      const c = profile.collection;
    ...
    return { ok: true, shards, earnedShards, capped: shards < earnedShards || !!battle && collectionRewards.gold < battle.detail.earned.gold, record, newly, collectionRewards, duplicate: false,
      ...(battle ? { battleReward: battle.detail, earnedGold: battle.detail.earned.gold } : {}) };
  }
  ...
  return Object.freeze({ VERSION, ECONOMY_VERSION, MAX_SHARDS, MAX_LEVEL, MAX_COUNTER, DECK_SIZE, MODES, MILESTONES, GEM_MILESTONES, migrateEconomy,
    BATTLE_REWARD_VERSION, BATTLE_MAX_SECONDS, BATTLE_REWARDS, rewardView,
